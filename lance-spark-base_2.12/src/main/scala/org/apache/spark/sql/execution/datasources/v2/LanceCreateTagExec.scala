...
import org.apache.spark.sql.catalyst.plans.logical.LanceCreateTagOutputType
...
case class LanceCreateTagExec(
  ...
  override protected def run(): Seq[InternalRow] = {
    ...
    try {
      val alreadyExists = dataset.tags().list().asScala.exists(_.getName == tagName)
      if (!ifNotExists || !alreadyExists) {
        dataset.tags().create(tagName, ref)
      }
    } finally {
      dataset.close()
    }
