...
case class LanceCreateTag(
  ...
  override def simpleString(maxFields: Int): String = {
    s"CreateTag(${tagName}, ${ref}, ifNotExists=${ifNotExists})"
  }

  override protected def withNewChildrenInternal(newChildren: IndexedSeq[LogicalPlan])
      : LanceCreateTag = {
    copy(newChildren(0), this.tagName, this.ref, this.ifNotExists)
  }
}

object LanceCreateTagOutputType {
