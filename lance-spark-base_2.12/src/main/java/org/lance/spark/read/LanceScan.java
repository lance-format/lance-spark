...
public class LanceScan
  ...
  public java.util.Optional<LanceNativeScanFallbackReason> nativeScanFallbackReason() {
    ...
    if (readOptions.getFullTextQuery() != null) {
      return fallback(
          ...
          "full-text search is not representable in native scan descriptor v1");
    }
    LanceRef ref = readOptions.getRef();
    if (ref == null || ref.getVersionNumber().isEmpty()) {
      return fallback(
  ...
  private long resolvedVersion() {
    return readOptions.getRef().getVersionNumber().get();
  }
