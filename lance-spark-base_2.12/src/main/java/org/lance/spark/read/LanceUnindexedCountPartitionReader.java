...
/**
 * Partition reader for unindexed fragments that uses normal scan-based counting. Each partition
 * contains a subset of fragments to count.
 */
public class LanceUnindexedCountPartitionReader implements PartitionReader<ColumnarBatch> {
  private final LanceSplitCountScan.LanceUnindexedCountPartition partition;
  private final BufferAllocator allocator;
  private boolean finished = false;
  private ColumnarBatch currentBatch;

  public LanceUnindexedCountPartitionReader(
      LanceSplitCountScan.LanceUnindexedCountPartition partition) {
  ...
  private long computeCount() {
    ...
    try (Dataset dataset = Dataset.open(allocator, uri, options)) {
      List<Integer> fragmentIds = partition.getFragmentIds();
      if (fragmentIds.isEmpty()) {
        return 0;
      }

      ScanOptions.Builder scanOptionsBuilder = new ScanOptions.Builder();
      if (partition.getWhereCondition().isPresent()) {
        scanOptionsBuilder.filter(partition.getWhereCondition().get());
      }
      ...
      try (LanceScanner scanner = dataset.newScan(scanOptionsBuilder.build())) {
        ...
        throw new RuntimeException("Failed to scan fragments " + fragmentIds, e);
      }
    }
