...
public class LanceScanTest {
  ...
  public void testNativeScanFallbackReasons() {
    FallbackCase[] cases =
        new FallbackCase[] {
          ...
          new FallbackCase(
              "full-text search",
              this::buildFullTextSearchScan,
              LanceNativeScanFallbackReasonCode.FULL_TEXT_SEARCH),
          new FallbackCase(
              "missing version",
  ...
  private LanceScan buildFullTextSearchScan() {
    LanceSparkReadOptions readOptions =
        LanceSparkReadOptions.builder()
            .datasetUri(TestUtils.TestTable1Config.datasetUri)
            .fullTextQuery(FullTextQuery.match("hello world", "body"))
            .build();
    return buildScan(readOptions, Collections.emptyMap(), null, Collections.emptyMap());
  }
  ...
  private LanceSparkReadOptions resolvedReadOptions() {
    LanceSplit.ScanPlanResult plan = LanceSplit.planScan(TestUtils.TestTable1Config.readOptions);
    return TestUtils.TestTable1Config.readOptions.withRef(plan.getRef());
  }
