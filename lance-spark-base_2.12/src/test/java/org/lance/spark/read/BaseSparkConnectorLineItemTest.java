...
public abstract class BaseSparkConnectorLineItemTest {
  ...
  static void setup() {
    ...
    assumeTrue(
        parquetPath != null && !parquetPath.isEmpty(),
        "PARQUET_PATH environment variable is not set");
    ...
    lanceData =
        spark
            ...
            .option(
                ...
                TestUtils.getDatasetUri(dbPath, "lineitem_10"))
            .load();
  ...
  public void test() {
    ...
    validateResults(
        data -> data.select("l_orderkey", "l_partkey", "l_quantity", "l_extendedprice").limit(10));
    validateResults(data -> data.groupBy("l_linestatus").avg("l_discount"));
    validateResults(
        data ->
            data.groupBy("l_partkey").sum("l_quantity").orderBy(desc("sum(l_quantity)")).limit(5));
    validateResults(data -> data.select("l_shipmode").distinct());
    validateResults(
        data -> data.select("l_orderkey", "l_comment").filter("l_comment LIKE '%express%'"));
    ...
    validateResults(
        data -> data.filter("l_quantity > 30").select("l_orderkey", "l_partkey", "l_quantity"));
    ...
    Function<Dataset<Row>, Dataset<Row>> function =
        data -> data.select("l_orderkey", "l_commitdate").orderBy("l_commitdate").limit(10);
    ...
    validateResults(
        data ->
            data.groupBy("l_orderkey")
                .sum("l_extendedprice")
                .orderBy(desc("sum(l_extendedprice)")));
  ...
  public void sql() {
    ...
    validateSQLResults(
        "SELECT l_shipmode, COUNT(*) AS count FROM parquet_dataset GROUP BY l_shipmode");
    validateSQLResults(
        "SELECT l_orderkey, SUM(l_extendedprice) AS total_extendedprice FROM parquet_dataset GROUP BY l_orderkey ORDER BY total_extendedprice DESC LIMIT 10");
    validateSQLResults(
        "SELECT l_suppkey, SUM(l_tax) AS total_tax FROM parquet_dataset GROUP BY l_suppkey ORDER BY total_tax DESC LIMIT 5");
    validateSQLResults(
        "SELECT l_orderkey, year(l_shipdate) AS ship_year FROM parquet_dataset GROUP BY l_orderkey, ship_year ORDER BY ship_year LIMIT 10");
    validateSQLResults(
        "SELECT l_orderkey, l_partkey, l_quantity FROM parquet_dataset WHERE l_quantity IS NULL");

    // LanceError(IO): Received literal Float64(100000) and could not convert to literal of type
    // 'Decimal128(15, 2)', rust/lance/src/datafusion/logical_expr.rs:28:17
    // spark.sql("SELECT * FROM lineitem WHERE (l_extendedprice <= 100000)").show();
    // spark.sql("SELECT * FROM lineitem2 WHERE (l_quantity > 30) AND (l_extendedprice <= 100000)
    // AND (l_comment IS NOT NULL)").show();
    // spark.sql("SELECT * FROM lineitem WHERE (l_quantity > 30) AND (l_extendedprice <
    // 50000)").show();
    // spark.sql("SELECT * FROM lineitem WHERE NOT (l_quantity > 30) AND ((l_comment IS NOT NULL) OR
    // (l_address IS NULL)) AND ((l_extendedprice < 100000) AND (l_extendedprice >=
    // 50000))").show();
    validateSQLResults(
        "SELECT * FROM parquet_dataset WHERE (l_quantity > 30) AND (l_comment IS NOT NULL)");
  }
  ...
  private void validateResults(Function<Dataset<Row>, Dataset<Row>> operation) {
    ...
    assertEquals(
        resultParquet.collectAsList(),
        resultLance.collectAsList(),
        "Results differ between Lance and Parquet datasets");
  }
  ...
  private void validateSQLResults(String sqlQuery) {
    ...
    assertEquals(
        resultParquet.collectAsList(),
        resultLance.collectAsList(),
        "Results differ between Lance and Parquet datasets for query: " + sqlQuery);
  }
}
