...
- **Compaction**: [OPTIMIZE](./optimize.md) replaces the fragments it compacts with new ones. Whether
  ...
  `fragments_indexed = 0`.
- **Stale `zonemap` Coverage**: a partially covered `zonemap` index falls back to scanning uncovered
  fragments in full, so query results stay complete but filtered queries do more I/O than a fully
  covered index. Refresh after appending or compacting to restore selective reads.
