 ...
 *
 * A single native `LanceProbe.probeRows(...)` call is already a complete distributed search: Lance
 * probes the IVF index and scans the candidate fragments across its own threads, heap-merges in
 * process, and returns the final top-K for one query — each hit already carrying its projected
 * payload. Handing that orchestration to Spark (a probe → shuffle → merge → materialize pipeline)
 * would only add a shuffle round-trip, a redundant merge stage, a second materialize scan, and
 * `M × N_frag × K` refs crossing the Rust→JVM boundary. So this stage keeps everything local:
 *
 * {{{
 ...
 *       // Search + project payload in ONE scan; Lance returns the final top-k best-first.
 ...
 * `runPartition`, so the probe semantics stay defined in exactly one place.
 */
object LanceKnnJoinStage {

  /**
   * Everything a probe task needs, shipped from the driver. `k` is the number of nearest neighbors
   * per left row. `leftVecIdx` is the position of the query-vector column in the left row.
   *
   ...
   * real per-row probe expansion through here.
   */
  ...
  /**
   * Expand one left row into its join output rows: search R's index for the top-`k` and assemble
   * `left ++ right ++ score` per hit. Returns an empty iterator (inner join) or a single null-right
   * row (outer join) when there is no query vector or no hit.
   */
  private def processRow(leftRow: Row, probe: LanceProbe, conf: Conf): Iterator[Row] = {
    ...
    if (q == null) {
      ...
      // Search R's index AND project the payload in ONE native scan. Lance returns the final top-`k`
      // best-first per query, so there is no JVM-side over-fetch, trim, or second materialize scan:
      // every hit already carries its right-side payload.
